...
var CURRENT_USER_KEY='fangwu_current_user';
...
function doLogin(){
  ...
  if(users[u]&&users[u].pwd===p){
    localStorage.setItem(CURRENT_USER_KEY,JSON.stringify(users[u]));
    updateNavUser();
...
function updateNavUser(){
  var u=localStorage.getItem(CURRENT_USER_KEY);
...
function renderProfile(){
  ...
  if(!u)return gp('home');
  var user=JSON.parse(u);
  return '<div class="admin-box"><h2>个人中心</h2><p>用户名: '+Object.keys(getUsers()).find(function(k){return getUsers()[k].phone===user.phone})+'</p><p>手机: '+user.phone+'</p><p>注册日期: '+user.date+'</p><button class="btn btn-primary" onclick="doLogout()">退出登录</button></div>';
}
